      ...
      function handleRoute() {
        ...
        switch (true) {
          ...
          case url.match(/\/accounts\/\d+$/) && method === "DELETE":
            return deleteUser();
          default:
            //pass through any requests not handled
        ...
        function refreshToken() {
          ...
          //remove the existing refresh token and generate a new one
        ...
        function revokeToken() {
          ...
          //get the current refresh token and find the user with that token
          const refreshToken = getRefreshToken();
          const user = users.find((x) =>
            x.refreshTokens.includes(refreshToken)
          );

          user.refreshTokens = user.filter((x) => x !== refreshToken);
        ...
        function getUserById() {
          ...
          if (
            !user ||
            (user.id !== currentUser().id && !isAuthorized(Role.Admin))
          ) {
            return unauthorized();
          }
          return ok(user);
